...
for q in range(epocas):
    ...
    somatoria = [somatori0(norm_x1, norm_x2, pesos[j]) for j in range(2)]
    somatoria_N1_N2 = list(zip(somatoria[0], somatoria[1]))
    # v_esperado do Neuronio 1 e 2 - usando ativação segmoid
    # N1 =1/1+e**(somatoria_N1_N2 + bias[0][0])
    # N1 =1/1+e**(somatoria_N1_N2 + bias[0][1])
    saida_N1_N2 = saidaN(somatoria_N1_N2, bias[0])

    #Calculando somatorio neuronio 3 = (N1*peso[1][0]+N2*peso[1][1])
    somatoria_N3 = [somatori0(norm_x1, norm_x2, saida_N1_N2[i]) for i in range(len(saida_N1_N2[0]))]
    somatorio_N3 = list(zip(somatoria_N3[0], somatoria_N3[1]))
