...
for q in range(epocas):
    for it in range(len(x1)):
        ...
        derivadaSaidaN3 = derivadaSigmoid(saida_N3)
        
        for n in range(2):
            pesos[n][0] = pesos[n][0] + tx_aprend * erro_peso_N1 * derivadaSaidaN3 * norm_x1[it]
        for n in range(2):
            pesos[n][1] = pesos[n][1] + tx_aprend * erro_peso_N1 * derivadaSaidaN3 * norm_x2[it]
        
        pesos[2][0] = pesos[2][0] + tx_aprend * erro * derivadaSaidaN3 * saida_N1 
        ...
        #g² = derivadaErro / derivadaBias²
        #vai gerar denominador zero algumas vezes:
        # bias[0][0] = bias[0][0] - (tx_aprend*(derivadaSigmoid(erro_peso_N1)/derivadaSigmoid(bias[0][0])))
        # bias[0][1] = bias[0][1] - (tx_aprend*(derivadaSigmoid(erro_peso_N2)/derivadaSigmoid(bias[0][1])))
